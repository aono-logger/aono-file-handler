...
var LogstashFormatter = /** @class */ (function () {
    function LogstashFormatter(prefix, consts) {
        if (prefix === void 0) { prefix = 'aono_'; }
    ...
    LogstashFormatter.prototype.format = function (entry) {
        ...
        var message = '{ ' +
            ...
            Object.keys(this.consts)
                .map(function (key) { return ", \"" + _this.prefix + key + "\": " + safeJsonStringify(_this.consts[key]); })
                .join('') +
            Object.keys(entry.meta)
                .map(function (key) { return ", \"" + _this.prefix + key + "\": " + safeJsonStringify(entry.meta[key]); })
                .join('') +
            ' }\n';
